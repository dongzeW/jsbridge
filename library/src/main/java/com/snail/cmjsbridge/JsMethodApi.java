...
class JsMethodApi {
   ...
   * js调用native，可能需要回调
   */
  @JavascriptInterface public void callNative(String jsonString) {
   ...
   * js调用native，可能需要回调
   */
  @JavascriptInterface public String syncCallNativeWithReturn(String jsonString) {
    if (TextUtils.isEmpty(jsonString)) {
      return null;
    }
   ...
   */
  @JavascriptInterface public void notifyNativeCallBack(String jsonString, int messageId) {
    if (TextUtils.isEmpty(jsonString)) {
      return;
    }
